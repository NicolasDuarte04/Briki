...
-- =====================================================
-- pgcrypto ya debe estar habilitada para encrypt_pii/decrypt_pii
...
CREATE TABLE IF NOT EXISTS public.companies (
  ...
  -- ==========================================
  -- SOFT DELETE (Opcional para compliance)
  -- ==========================================
...
CREATE INDEX IF NOT EXISTS idx_companies_is_pinned ON public.companies(org_id, is_pinned) WHERE is_pinned = TRUE;
CREATE INDEX IF NOT EXISTS idx_companies_risk_classification ON public.companies(org_id, risk_classification);
CREATE INDEX IF NOT EXISTS idx_companies_company_type ON public.companies(org_id, company_type);
...
-- Política 4: DELETE - Solo admin puede eliminar (soft delete)
...
COMMENT ON POLICY "companies_org_isolation_delete" ON public.companies IS 
  'Solo permite a administradores eliminar empresas (DELETE)';
...
COMMENT ON FUNCTION public.soft_delete_company IS 
  'Realiza soft delete de una empresa. Requiere permisos de admin por RLS.';
...
BEGIN
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    ELSIF NEW.risk_classification != OLD.risk_classification THEN
      INSERT INTO public.audit_log (
        org_id, user_id, actor, action, resource_type, resource_id, payload
      ) VALUES (
        NEW.org_id,
...
DO $$
BEGIN
  RAISE NOTICE '✅ Migración 20260210_create_companies_table completada exitosamente';
  RAISE NOTICE '   - Tabla companies creada con campos PII encriptados';
  RAISE NOTICE '   - RLS habilitado con políticas por rol';
