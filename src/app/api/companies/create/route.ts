...
export async function POST(request: NextRequest) {
  try {
    ...
    const companyData: CreateCompanyInput = {
      ...
      ...(headquartersAddress && { headquartersAddress: headquartersAddress.trim() }),
      ...
      ...(financialCutDate && { financialCutDate: new Date(financialCutDate) }),
      ...(annualPayroll && { annualPayroll: annualPayroll.trim() }),
      ...(employeeCount !== undefined && employeeCount !== null && { employeeCount: Number(employeeCount) }),
      ...(currency && { currency }),
      ...(riskClassification && { riskClassification: riskClassification as RiskClassification }),
      ...(ciiuCode && { ciiuCode: ciiuCode.trim() }),
      ...(activityDescription && { activityDescription: activityDescription.trim() }),
      isPep: isPep ?? false,
