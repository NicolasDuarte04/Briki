...
interface ComparisonTableProps {
    ...
    baselineAnalysisIds?: Set<string>; // ✅ MULTI-BASELINE: Set de IDs baseline
}

export function ComparisonTable({ comparison, analyses, baselineAnalysisIds }: ComparisonTableProps) {
    ...
    // ✅ MULTI-BASELINE: Reordenar columnas — baselines primero, challengers después
    // Solo reorden visual; comparison.analysisIds original no se muta
    const sortedAnalysisIds = baselineAnalysisIds && baselineAnalysisIds.size > 0
        ? [
            ...comparison.analysisIds.filter(id => baselineAnalysisIds.has(id)),
            ...comparison.analysisIds.filter(id => !baselineAnalysisIds.has(id)),
          ]
        : comparison.analysisIds;

    // ✅ FIX: Número fijo de columnas para mostrar TODAS las pólizas
    const columnCount = sortedAnalysisIds.length;
    const gridTemplateColumns = `200px repeat(${columnCount}, minmax(220px, 1fr))`;

    // ✅ Traducción de categorías via i18n
                        ...
                        <div 
                            ...
                            className={`p-4 border-l border-border/50 flex flex-col gap-1 ${
                                isBaseline 
                                    ...
                                    : 'bg-emerald-50/40 dark:bg-emerald-950/15 border-l border-l-emerald-300/50'
                            }`}
                        >
                            <div className="flex items-center gap-2 mb-1">
                                <Checkbox
                                    ...
                                    aria-label={`${t("includeInProposal")} ${insurerName}`}
                                />
                                <label
                                    htmlFor={`select-${id}`}
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                {isBaseline ? (
                                    ...
                                    <Badge variant="secondary" className="bg-emerald-100 text-emerald-700 dark:bg-emerald-900 dark:text-emerald-300 text-[10px] flex items-center gap-1">
                            ...
                            <div key={category} className="flex flex-col">
                                ...
                                {(groupedRows[category] || []).map((row, index) => (
                                    <ComparisonRow
                                        key={row.id}
                                        row={row}
                                        analysisIds={sortedAnalysisIds}
                                        columnCount={columnCount}
                                        isEven={index % 2 === 0}
                                        {...(baselineAnalysisIds && baselineAnalysisIds.size > 0 && { baselineAnalysisIds })}
                                    />
                                ))}
                            </div>
                        ))}
