...
-- - Las funciones se actualizan SOLO para agregar SET search_path
...
-- Solo agregamos SET search_path = '' para prevenir SQL injection
-- MANTENEMOS la implementación original intacta
-- ============================================================================

-- 1. encrypt_pii - usar extensions.pgp_sym_encrypt (pgcrypto está en schema extensions)
...
BEGIN
    IF data IS NULL THEN
        RETURN NULL;
    END IF;
    RETURN extensions.pgp_sym_encrypt(data, current_setting('app.encryption_key'));
END;
$$;

-- 2. decrypt_pii - usar extensions.pgp_sym_decrypt
...
BEGIN
    IF data IS NULL THEN
        RETURN NULL;
    END IF;
    RETURN extensions.pgp_sym_decrypt(data, current_setting('app.encryption_key'));
END;
$$;
...
-- 7. update_compliance_updated_at
...
-- 8. encrypt_api_key - DROP + CREATE para cambiar nombre de parámetro
-- Usa extensions.pgp_sym_encrypt (pgcrypto está en schema extensions)
...
BEGIN
    IF plain_key IS NULL THEN
        RETURN NULL;
    END IF;
    RETURN extensions.pgp_sym_encrypt(plain_key, current_setting('app.encryption_key'));
END;
$$;

-- 9. decrypt_api_key - DROP + CREATE para cambiar nombre de parámetro
-- Usa extensions.pgp_sym_decrypt
...
BEGIN
    IF encrypted_key IS NULL THEN
        RETURN NULL;
    END IF;
    RETURN extensions.pgp_sym_decrypt(encrypted_key, current_setting('app.encryption_key'));
END;
$$;

-- ============================================================================
-- PARTE 2: CORREGIR VIEWS CON SECURITY DEFINER
...
-- ==================== PROFILES ====================
-- Eliminar todas las políticas existentes y recrear optimizadas
...
-- ==================== CASE_POLICY_LINKS ====================
-- NOTA: Esta tabla puede no existir aún. Las políticas se aplican condicionalmente.
DO $$
BEGIN
    ...
    ELSE
        RAISE NOTICE '⚠️ Tabla case_policy_links no existe aún - políticas se aplicarán cuando se cree la tabla';
    END IF;
END $$;
...
-- ============================================================================
-- PARTE 4: CREAR ÍNDICES PARA MEJORAR RENDIMIENTO DE POLÍTICAS RLS
-- ============================================================================

-- Índice compuesto para org_members (usado en casi todas las políticas RLS)
...
-- Índice para case_policy_links (condicional - tabla puede no existir)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'case_policy_links') THEN
...
BEGIN
    ...
    RAISE NOTICE '- Funciones de encriptación actualizadas con search_path seguro';
    RAISE NOTICE '- Vistas actualizadas con SECURITY INVOKER';
